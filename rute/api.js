...
try {
var zahirr = db.get("zganz404");
} catch (e) {
	console.log('')
}

var creator = "@zganz404"
...
const listkey = ["kontolodon", "apikey20"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'kontolodon') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'kontolodon')  return res.json(loghandler.invalidKey)
